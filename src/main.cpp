...
int main() {
  ...
  // std::cout << "CPU: "; for (auto pid : LinuxParser::Pids()) { std::cout << pid << " "; }; std::cout << '\n';
  // std::cout << "Memory: " << std::fixed << std::setprecision(3)
  //           << LinuxParser::MemoryUtilization() << '\n';
  // std::cout << "CPU Utilization: "; for (auto state : LinuxParser::CpuUtilization()) {std::cout << state << " "; }; std::cout << '\n';
